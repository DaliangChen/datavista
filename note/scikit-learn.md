...
- 目标与原理：
  - OLS 的核心目标是最小化残差平方和。
  - 通过拟合一个线性模型，它力求使实际观测到的目标值与模型预测值之间的平方差之和达到最小。
- 系数确定：
  - 通过解决一个特定的最小化问题，OLS 能够计算出线性模型中各个特征的最佳系数。
- 特征独立性：
  - OLS 模型在特征之间相互独立（无相关性）时表现最佳，此时模型性能和稳定性都较好。
- 多重共线性问题：
  - 当特征之间存在高度相关性（即“多重共线性”）时，设计矩阵可能会变得接近奇异。
  - 这会导致最小二乘估计对随机误差非常敏感，从而使得系数的方差显著增大，影响模型的稳定性和解释性。
- 非负最小二乘法：
  - OLS 的一个变体允许将所有模型系数限制为非负值。
  - 这在处理某些自然上不能为负的量（例如，物理量、数量等）时非常有用。

### 1.1.2. Ridge regression and classification

- 目的：通过对系数的大小施加惩罚来解决多重共线性问题，并提高模型的稳定性。
- 原理：它最小化的是一个带惩罚项的残差平方和。这个惩罚项会缩小系数（“收缩”系数），使其对共线性不那么敏感。
- 复杂性参数 (alpha, α)：
  - 惩罚的强度由参数 `alpha` (α) 控制。
  - `alpha` 值越大，对系数的惩罚越强，系数收缩得越多，模型对共线性的鲁棒性越好。

### 1.1.3. Lasso

- 核心目标：
  - Lasso 模型的目的是最小化一个结合了最小二乘惩罚和 L1-范数正则化项的函数。
- L1-范数正则化：
  - L1-范数惩罚促使模型的一些系数精确地变为零。
  - 这种特性使得 Lasso 能够自动执行特征选择，因为它实际上减少了模型所依赖的特征数量。
- `alpha` 参数：
  - `alpha` 参数控制着 L1 惩罚的强度。
  - `alpha` 值越大，对系数的惩罚就越强，导致更多的系数被压缩为零，从而产生更稀疏的模型。
