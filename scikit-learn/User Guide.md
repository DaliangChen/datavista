...
$$
\hat{y}(\mathbf{w}, \mathbf{x}) = w_0 + w_1 x_1 + w_2 x_2 + \dots + w_p x_p
$$

这些模型的核心差异就在于它们最小化的损失函数不同，一般形式如下：

$$
\min_{\mathbf{w}, b} \; \underbrace{L(\hat{y}, y)}_{\text{损失函数}} + \underbrace{R(\mathbf{w})}_{\text{正则化项（可选）}}
$$

其中：

- $L(\hat{y}, y)$：衡量预测值 $\hat{y}$ 与真实值 $y$ 的差异
- $R(\mathbf{w})$：正则化项（例如 L1、L2），用于控制模型复杂度、防止过拟合

注意的点：

- 虽然所有这些模型的输出都是线性组合，但某些模型会对输出再加一层变换，例如：

  - `LogisticRegression`: 输出通过 sigmoid 函数 $\sigma(z)$
  - `SGDClassifier`: 可选择 log loss（对数几率）或 hinge（SVM）

## 1.2. Linear and Quadratic Discriminant Analysis

适用于数据服从高斯分布的情况，它们通过对各类别的概率密度建模，计算后验概率，从而完成分类。

## 1.3. Kernel ridge regression

数据特征与目标变量之间存在复杂非线性关系，但又希望保留线性模型的简洁性。使用合适的核函数（如 RBF、多项式、sigmoid 等），KRR 可以灵活拟合各种模式的目标函数。
...
### 1.7.2. Gaussian Process Classification
...
### 2.2.8. Multi-dimensional Scaling

### 2.2.9. t-distributed Stochastic Neighbor Embedding

### 2.2.1. Tips on practical use
...
### 2.3.1. Clustering performance evaluation
...
### 2.5.7. Non-negative matrix factorization

### 2.5.8. Latent Dirichlet Allocation
...
### 5.1.2. Individual conditional expectation
...
![alt text](<User Guide/ml_map.svg>)
